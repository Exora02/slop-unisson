...
Future<void> probe(String url, String ua, String range) async {
  ...
  stdout.writeln('  "$range" -> ${resp.statusCode} '
      'content-range=${resp.headers.value(HttpHeaders.contentRangeHeader) ?? "-"}');
}

Future<void> main() async {
  final c = InnerTubeClient();
  final r = (await c.resolve('jWs_EsTFUi8', force: true))!;
  stdout.writeln('via ${r.clientUsed}');
  await probe(r.url, r.userAgent, 'bytes=0-0');
  await probe(r.url, r.userAgent, 'bytes=0-1');
